...
class SleepInterval
  attr_accessor :id, :start, :finish, :day
  
  def initialize(guard_id, sleep_time, wake_time)
    self.id = guard_id
    self.start = MyDate.new(sleep_time)
    self.finish = MyDate.new(wake_time)
    self.day = self.start.day
  end

  def total_min
    self.finish.min - self.start.min
  end

  def minute_list
    (self.start.min .. self.finish.min - 1).to_a
  end
end

class MyDate
  attr_accessor :day, :min

  def initialize(datestring)
    self.day, self.min = self.class.parse_date(datestring)
  end

  def self.parse_date(datestring)
    match = datestring.match(/(....-..-..) ..:(..)/)
    date, min = match.captures
    [date, min.to_i]
  end
end

def parse_sleeps(log_list)
  # read an unsorted log list and produce a hash of SleepIntervals per guard
  sleeps = {}
  current_guard = nil
  sleepy_time = nil
  wakey_time = nil
  log_list.sort.each do |log_line|
    if match = log_line.match(/Guard #([^ ]*) begins shift/)
      current_guard = match.captures[0].to_i
      puts "guard #{current_guard} on shift"
    elsif match = log_line.match(/\[(.*)\] falls asleep/)
      sleepy_time = match.captures[0]
      puts "sleeps at #{sleepy_time}"
    elsif match = log_line.match(/\[(.*)\] wakes up/)
      wakey_time = match.captures[0]
      puts "wake up at #{wakey_time}, time to make interval with #{current_guard}, at #{sleepy_time}"
      #binding.pry
      sleep_amount = SleepInterval.new(current_guard, sleepy_time, wakey_time)
      sleeps[current_guard] ||= []
      sleeps[current_guard].push(sleep_amount)
    end
  end
  sleeps
end
...
if __FILE__ == $0
  ...
  sleeps = parse_sleeps(DATA.readlines)

  sleep_amounts = sleeps.map do |guard, list|
    [guard, list.reduce(0) { |sum, interval| sum + interval.total_min }]
  end
  most_sleep = sleep_amounts.sort_by { |pair| pair[1] }.pop

  puts "guard #{most_sleep[0]} slept the most at #{most_sleep[1]} mins"

  sleepy_guard = most_sleep[0]
  which_minute = sleeps[sleepy_guard].flat_map { |interval| interval.minute_list }.group_by(&:itself).map{ |k, list| [k, list.count] }.sort_by { |k,v| v }.pop.first
  puts "guard #{sleepy_guard} slept the most at minute #{which_minute}"

  puts "part 1: #{sleepy_guard} * #{which_minute} = #{sleepy_guard*which_minute}"
  puts
  # find the guard that is asleep the most on some minute, and which minute that is.
  minute_sleeps = {}
  sleeps.each do |g, list|
    # for each guard, process their list and add that guard to the minute_sleeps list for those minutes
    frequent_minute_list = list.flat_map { |interval| interval.minute_list }.group_by(&:itself).values.sort_by { |x| x.count }.pop
    minute_sleeps[frequent_minute_list.first] = [g, frequent_minute_list.count]
  end
  answer = minute_sleeps.sort_by { |minute, pair| pair[1] }.pop
  minute = answer[0]
  guard = answer[1][0]
  puts "part 2: guard #{guard} slept #{answer[1][1]}x on minute #{minute}"
  puts "#{guard} * #{minute} = #{guard*minute}"
end
